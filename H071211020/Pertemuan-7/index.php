...
<body>
    ...
    <div class="d-flex justify-content-center flex-column align-items-center mt-5">
        ...
        <table class="table w-75">
            ...
            <tbody>
                ...
                if(isset($_GET['search'])){
                    $dataMahasiswa = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3306), "select * from `data` where CONCAT(NIM, Nama, Alamat, Fakultas) like '%" .$_GET['search']. "%' limit $posisi, $batas");
                } else {
                    $dataMahasiswa = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3306), "select * from `data` limit $posisi, $batas");
                }
        ...
        <?php
            ...
            if(isset($_GET['search'])){
                $search = $_GET['search'];
                $sqlPage = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3306), "select * from `data` where CONCAT(NIM, Nama, Alamat, Fakultas) like '%" .$_GET['search']. "%'");
                $jumlahData = mysqli_num_rows($sqlPage);
                $jumlahHalaman = ceil($jumlahData/$batas);
            } else {
                $sqlPage = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3306), "select * from `data`");
                $jumlahData = mysqli_num_rows($sqlPage);
                $jumlahHalaman = ceil($jumlahData/$batas);
            }
        ?>
                ...
                <div class="modal-body">
                    <form action="backend.php?tambahDataMahasiswa" method="POST">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-10">
                            <select class="form-select" name="Fakultas" aria-label="Default select example" required>
                                <option selected disable value ="">Open this select menu</option>
                                <option value="Ekonomi & Bisnis">Ekonomi & Bisnis</option>
                                <option value="Hukum">Hukum</option>
                                <option value="Kedokteran">Kedokteran</option>
                                <option value="Teknik">Teknik</option>
